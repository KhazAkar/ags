...
export default function DisplayEditComponent() {
    ...
    let [ _ , setinnerReload] = useState<any>("");
        ...
        <div className={"nameHolder"}>
            {
                isEdit === false ? (
                    <div className={"nameHolderForActivate"} >
                        ...
                        <ButtonComp text={"Edit name"} onClickFun={() => { setNewName(sliderData.controlPanelData[sliderData.currentIndex].profileName); toggleState(true) }} svgPath={svgEdit} upperCol={"rgb(15, 207, 255)"} underCol={"rgb(46, 110, 247)"} />

                        <ButtonComp onClickFun={
                            () => {
                    ...
                    <div className={"nameHolderFor3"} >
                        ...
                        />
                        <ButtonComp text={"save name"} onClickFun={() => {
                            saveNewName();
                            toggleState(false);
                            reloader(newName);
                        }}
                            svgPath={svgSave} />
                        <ButtonComp text={"Edit name"} onClickFun={() => { toggleState(false) }} svgPath={svgCancel} upperCol={"rgb(241, 25, 61)"} underCol={"rgb(177, 23, 48)"} />
                    </div>
                    )
            }
        </div>
