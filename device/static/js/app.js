...
async function sendControlCommand(device, action) {
    ...
    try {
        ...
        // Update button text
        if (button) {
            button.textContent = `${device.charAt(0).toUpperCase() + device.slice(1)} ${action === 'on' ? 'On' : 'Off'}`.trim();
...
function updateSensorData(data) {
    ...
    // Update sensor cards
    updateSensorCard(elements.airTempCard, data.air_temperature, '°C', getTrendIndicator(data.air_temperature, previousData.air_temperature));
    updateSensorCard(elements.airHumidityCard, data.air_humidity, '%', getTrendIndicator(data.air_humidity, previousData.air_humidity));
    updateSensorCard(elements.soilTempCard, data.soil_temperature, '°C', getTrendIndicator(data.soil_temperature, previousData.soil_temperature));
    updateSensorCard(elements.soilMoistureCard, data.soil_moisture, '%', getTrendIndicator(data.soil_moisture, previousData.soil_moisture));
    updateSensorCard(elements.lightLevelCard, Math.round(data.light_level), 'lx', getTrendIndicator(data.light_level, previousData.light_level));
...
async function init() {
    ...
    // Initial data load
    await updateSensorData(await fetchSensorData());
